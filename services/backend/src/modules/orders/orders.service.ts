...
import { extractDesignFiles } from './order-design-files.js';
...
export class OrdersService {
  ...
  async transferToMember(id: string, input: TransferOrderToMemberInput) {
    ...
    if (!target) throw new NotFoundException('Follow-up assignee not found');
    const note = input.note.trim();
    if (!note) throw new BadRequestException('Follow-up instructions are required');
    ...
    const created = await this.prisma.db.serviceRequest.create({
      data: {
        ...
        metadata: {
          category: 'admin_order_follow_up',
    ...
    await this.prisma.db.serviceRequestComment.create({
      data: {
        ...
        body: `Order ${orderNumber} follow-up assigned: ${note}`,
        internal: true,
        attachmentsJson: [{
          kind: 'admin_order_follow_up',
          orderId: order.id,
          orderNumber,
          shopifyOrderId: order.shopifyOrderId,
        }] as Prisma.InputJsonValue,
