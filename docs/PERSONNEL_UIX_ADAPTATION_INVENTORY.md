...
| Priority customers | `SegmentOwnership -> SegmentCustomerMembership -> Customer` plus latest order/call/note context | Staff sees their owned customer portfolio grouped into customer lists | Our `segmentGroups` and `priorityKanban` remain the portfolio source internally. Staff-visible copy says Priority customers / customer lists, never raw kanban/segment language | Preserve; never populate Priority from Daily task query |
...
- Priority customers is the staff member's assigned Shopify segment portfolio.
  It remains segment-grouped internally, but staff-visible UI says customer
  lists and changes as Shopify segment membership changes.
...
| Customer-list navigation | `kanban-v2` panel with All lists / List N chips and previous/next list navigation | Priority customers still come only from `SegmentOwnership -> SegmentCustomerMembership -> Customer`; staff copy does not expose raw segment/kanban terms |
| Priority customer cards | `segment-customer-card card-v2` with avatar, phone/email, latest note, latest order, latest call, open follow-up summary, call/note/pin actions | Uses live `segmentGroups` items. Clicking opens Customer 360 popup with real detail API data |
...
| Priority customers | `CallQueue.tsx` | Staff's assigned customer portfolio by customer list | Personnel | SegmentOwnership -> SegmentCustomerMembership -> Customer | Exists | Preserve internal segment grouping and enrich with risk fields; render as customer lists in staff UI | Priority customers | fields/copy/theme | Screenshot with customer lists |
