...
import { personTokenStore, subscribeSession } from './api';
...
export function subscribePersonWorkspaceRealtime(listener: RealtimeListener) {
...
function ensureSocket() {
  if (socket) return;
  const token = personTokenStore.getAccessToken();
  if (!token) return;
...
function realtimeEndpoint() {
  const base = import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:4100/api/v1';
