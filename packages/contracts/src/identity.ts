...
export function resolveBrandLogoUrl(
  assets: Partial<BrandAssets> | null | undefined,
  fallbackUrl: string | null | undefined,
  surface: BrandLogoSurface,
) {
  const primary = assets?.primaryLogoUrl?.trim() || '';
  const dark = assets?.darkLogoUrl?.trim() || '';
  const fallback = fallbackUrl?.trim() || '';
  return surface === 'dark'
    ? dark || primary || fallback
    : primary || dark || fallback;
}
...
const accountPortalCompositionDefault = {
  desktopHeightMode: 'fixed' as const,
  desktopPagePadding: 40,
  desktopStageWidth: 1120,
  pageVerticalAlignment: 'center' as const,
  heroPanelPercent: 38,
  stageCornerRadius: 8,
  stageBorderWidth: 1,
  stageShadowBlur: 40,
  ...
  formPaddingTop: 38,
  formPaddingRight: 38,
  formPaddingBottom: 38,
  formPaddingLeft: 38,
  formBrandBottomGap: 22,
  formHeadingBottomGap: 20,
  formNoticeBottomGap: 12,
  formFieldRowGap: 14,
  formFieldColumnGap: 14,
  formLabelGap: 6,
  formInputPaddingY: 10,
  formInputPaddingX: 14,
  formButtonHeight: 46,
  formButtonTopGap: 22,
  formSigninTopGap: 16,
  formTextareaRows: 3,
...
const loginExperienceDefault = {
  ...
  heroBrandLayout: 'inline' as const,
  heroLogoSize: 'standard' as const,
  heroLogoSurface: 'auto' as const,
  formLogoSurface: 'auto' as const,
  heroBrandSize: 'standard' as const,
  heroVerticalAlignment: 'center' as const,
  heroPadding: 'standard' as const,
  heroContentGap: 'standard' as const,
  benefitDensity: 'standard' as const,
  desktopFit: false,
  desktopStageHeight: 680,
  formVerticalAlignment: 'center' as const,
  benefitsPlacement: 'flow' as const,
  ...accountPortalCompositionDefault,
  ...
  formLogoSize: 'standard' as const,
  formBrandAlignment: 'center' as const,
...
const accountPortalPageSchema = z.object({
  ...
  heroBrandLayout: z.enum(['inline', 'stacked']).default('inline'),
  heroLogoSize: z.enum(['standard', 'large']).default('standard'),
  heroLogoSurface: z.enum(['auto', 'light', 'dark']).default('auto'),
  formLogoSurface: z.enum(['auto', 'light', 'dark']).default('auto'),
  heroBrandSize: z.enum(['standard', 'large']).default('standard'),
  heroVerticalAlignment: z.enum(['top', 'center']).default('center'),
  heroPadding: z.enum(['compact', 'standard', 'spacious']).default('standard'),
  heroContentGap: z.enum(['tight', 'standard', 'open']).default('standard'),
  benefitDensity: z.enum(['compact', 'standard']).default('standard'),
  desktopFit: z.boolean().default(false),
  desktopStageHeight: z.number().int().min(560).max(900).default(680),
  formVerticalAlignment: z.enum(['top', 'center']).default('center'),
  benefitsPlacement: z.enum(['flow', 'lower']).default('flow'),
  desktopHeightMode: z.enum(['content', 'viewport', 'fixed']).default('fixed'),
  desktopPagePadding: z.number().int().min(0).max(80).default(40),
  desktopStageWidth: z.number().int().min(680).max(1440).default(1120),
  pageVerticalAlignment: z.enum(['top', 'center']).default('center'),
  heroPanelPercent: z.number().int().min(25).max(60).default(38),
  stageCornerRadius: z.number().int().min(0).max(32).default(8),
  stageBorderWidth: z.number().int().min(0).max(4).default(1),
  stageShadowBlur: z.number().int().min(0).max(100).default(40),
  stageShadowOpacity: z.number().int().min(0).max(30).default(14),
  mobilePagePadding: z.number().int().min(0).max(40).default(16),
  mobileHeroVisible: z.boolean().default(false),
  mobileFormPaddingTop: z.number().int().min(0).max(80).default(28),
  mobileFormPaddingRight: z.number().int().min(0).max(80).default(22),
  mobileFormPaddingBottom: z.number().int().min(0).max(80).default(28),
  mobileFormPaddingLeft: z.number().int().min(0).max(80).default(22),
  heroPaddingTop: z.number().int().min(0).max(160).default(38),
  heroPaddingRight: z.number().int().min(0).max(160).default(34),
  heroPaddingBottom: z.number().int().min(0).max(160).default(38),
  heroPaddingLeft: z.number().int().min(0).max(160).default(34),
  heroBrandGap: z.number().int().min(0).max(80).default(14),
  heroBrandToIntroGap: z.number().int().min(0).max(160).default(24),
  heroEyebrowToHeadlineGap: z.number().int().min(0).max(80).default(10),
  heroHeadlineToDescriptionGap: z.number().int().min(0).max(80).default(8),
  heroIntroToBenefitsGap: z.number().int().min(0).max(240).default(24),
  heroBenefitRowGap: z.number().int().min(0).max(80).default(16),
  heroTrustTopGap: z.number().int().min(0).max(120).default(20),
  formPaddingTop: z.number().int().min(0).max(160).default(38),
  formPaddingRight: z.number().int().min(0).max(160).default(38),
  formPaddingBottom: z.number().int().min(0).max(160).default(38),
  formPaddingLeft: z.number().int().min(0).max(160).default(38),
  formBrandBottomGap: z.number().int().min(0).max(100).default(22),
  formHeadingBottomGap: z.number().int().min(0).max(100).default(20),
  formNoticeBottomGap: z.number().int().min(0).max(80).default(12),
  formFieldRowGap: z.number().int().min(0).max(48).default(14),
  formFieldColumnGap: z.number().int().min(0).max(48).default(14),
  formLabelGap: z.number().int().min(0).max(24).default(6),
  formInputPaddingY: z.number().int().min(2).max(24).default(10),
  formInputPaddingX: z.number().int().min(4).max(32).default(14),
  formButtonHeight: z.number().int().min(32).max(64).default(46),
  formButtonTopGap: z.number().int().min(0).max(64).default(22),
  formSigninTopGap: z.number().int().min(0).max(48).default(16),
  formTextareaRows: z.number().int().min(2).max(8).default(3),
  ...
  footerPlacement: z.enum(['form', 'page']).default('form'),
  ...
  formLogoSize: z.enum(['compact', 'standard', 'large']).default('standard'),
  formBrandAlignment: z.enum(['left', 'center']).default('center'),
...
export const accountPortalExperienceSchema = z.object({
  ...
  requestAccess: accountPortalPageSchema.extend({
    desktopStageHeight: z.number().int().min(560).max(900).default(620),
    desktopPagePadding: z.number().int().min(0).max(80).default(20),
    desktopStageWidth: z.number().int().min(680).max(1440).default(980),
